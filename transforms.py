...
class ToTensor(object):
    ...
    def __init__(self, make_CHW=True, input_format=None, out_type=torch.float32):
    ...
    def __call__(self, x):
        ...
        if self.make_CHW:
            if self.input_format == "HW":
                ...
                raise NotImplementedError
        x = torch.from_numpy(x).contiguous().type(self.out_type)
        return x


class TimeSeriesToTensor(object):

    def __init__(self, make_TCHW=True, input_format=None, out_type=torch.float32):
